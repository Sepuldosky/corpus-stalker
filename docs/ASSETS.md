...
| `sound/npc/librarian/<accion>/*.ogg` (**0 — carpetas creadas y VACÍAS**, 2026-08-17) | Voz del **trader de comida** (roadmap [1]), mismo estándar por acción que Sidorovich y las mismas diez carpetas, con su `about.txt`. **Vacías a propósito**: una carpeta sin sonidos hace que el trader calle esa acción sin error, así que el tramo se puede cerrar antes de que exista un audio. El nombre de la carpeta es contrato con `ENT.VoiceDir = "npc/librarian"` — si se renombra una sin la otra, el trader queda **mudo sin dar error**. Ojo: el modelo del personaje se llama distinto (`hawaiian.mdl`) | **las aporta el autor**; todavía no hay ninguna |
